...
export default function LikedProduct() {
    ...
    const currentUser = useSelector((state) => state.currentUserReducer.liked)
        ...
        <>
            <div className="flex flex-col bg-neutral-900 lg:rounded-xl min-h-screen col-span-12 sm:col-span-8 font-Lato">
                <header className="flex justify-between items-center bg-slate-300 p-5 lg:rounded-t-lg">
                    <h1 className="text-3xl">Les produits que vous aimés</h1>
                    <Tooltip
                        content="Les produits affichés sont les articles que vous avez aimés"
                    >
                ...
                <section className="grid grid-cols-12 bg-neutral-900 p-5 gap-2 overflow-y-scroll">
                    {!isEmpty(allProduct) && !isEmpty(currentUser) ? (
                        allProduct.filter((product) => currentUser.map(likedProduct => likedProduct.product_id).includes(product.id)).map((product, index) => (
                            <div className="flex flex-col col-span-12 lg:col-span-6 xl:col-span-4 bg-slate-300 relative shadow w-full" key={index}>
                                <div className="col-span-4 w-full">
                                    <Carousel arrowSize={'20'}>
                                        {JSON.parse(product.url_image).map((image, index) => {
                                            return (
                                                <img key={index} src={`../../uploads/product/${image}`} alt="product image" className="min-w-full object-cover h-96 w-16" />
                                            )
                                        }
                                        )}
                                    </Carousel>
                                ...
                                <div className="flex flex-col justify-between p-5">
                                    <div className="flex justify-between items-start">
                                        ...
                                        <p className={`lowercase first-letter:uppercase px-3 py-1 font-SourceSansPro text-center opacity-95 text-sm ${product.category == "DIVERS" ? "bg-red-300" : product.category == "ACCESSOIRES" ? "bg-green-300" : product.category == "VÊTEMENTS" ? "bg-sky-300" : ""}`}>{product.category}</p>

                                    </div>
                                    <Accordion>
                                        <Accordion.Panel>
                                            <Accordion.Title>
                                                <h2 className="text-black">Description du produit</h2>
                                            </Accordion.Title>
                                            <Accordion.Content>
                                                <p className="mb-2 text-black overflow-y-scroll h-40">
                                                    {product.description}
                                                </p>
                                            </Accordion.Content>
                                        </Accordion.Panel>
